...
from lolram_deprecated_1 import serializer
from lolram.util import resoptimizer
...
class BaseView(object):
	...
	@classmethod
	def supports(cls, format): #@ReservedAssignment
		return 'to_%s' in dir(cls)
	
	@classmethod
	def render(cls, context, model, format, **opts): #@ReservedAssignment
...
class DocumentView(BaseView):
	...
	def to_html(cls, context, model, **opts):
		...
		if model.messages:
			...
			for title, subtitle, icon in model.messages: #@UnusedVariable
	...
	def to_html(cls, context, model):
		...
		for option in model.itervalues():
			...
			if dropdown_element:
				...
				input = lxmlbuilder.INPUT(type=input_type, id=model.name+name, #@ReservedAssignment
...
class FormView(BaseView):
	...
	def to_html(cls, context, model):
		...
		for key, widget in model.iteritems(): #@UnusedVariable
...
class ToUnicodeView(BaseView):
	...
	@classmethod
	def render(cls, context, model, format='html'): #@ReservedAssignment
