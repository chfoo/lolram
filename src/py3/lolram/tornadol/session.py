...
#
from lolram.tornado import Controller
from lolram.tornadol.database import DatabaseController
...
class SessionController(Controller):
    ...
    def session(self, request, key_name=KEY_NAME):
        collection = self.application.controllers[
            DatabaseController].db[SessionModel.COLLECTION]
        ...
        if not d.id \
        or (datetime.datetime.utcnow() - result[SessionModel.COOKIE_DATE]
            ).days > 10 \
    ...
    key_name=KEY_NAME):
        request.set_secure_cookie(
            key_name,
            session_id_str,
            expires_days=SessionController.MAX_AGE_DAYS if persistent \
                else None,
        )

    def _remove_old_sessions(self):
        date = datetime.datetime.fromtimestamp(
            time.time() - 3600 * SessionController.MAX_AGE_DAYS)
        ...
        self.application.controllers[DatabaseController].execute_when_ready(f)
