...
#        self.assertEqual(str(url), s)
    ...
    def test_collapse_path(self):
        ...
        s = '/a/b/c/../'
        self.assertEqual(lolram.url.collapse_path(s), '/a/b/')

        s = 'a/b/./c'
        self.assertEqual(lolram.url.collapse_path(s), 'a/b/c')

        s = '/a/b/c/..'
        self.assertEqual(lolram.url.collapse_path(s), '/a/b/')

        s = 'a/../b/c'
        self.assertEqual(lolram.url.collapse_path(s), 'b/c')
    ...
    def test_to_punycode_hostname(self):
        ...
        self.assertEqual(lolram.url.to_punycode_hostname(s),
            'aa.bb.cc.x--abcdef-mhab25gxirn86c.com')
        self.assertEqual(lolram.url.to_punycode_hostname('example.com'),
            'example.com')
    ...
    def test_from_punycode_hostname(self):
        ...
        self.assertEqual(lolram.url.from_punycode_hostname(s),
            'aa.bb.cc.ačbǔcŏdīe¤f¤.com')
        self.assertEqual(lolram.url.from_punycode_hostname('example.com'),
            'example.com')

    def test_is_allowable_hostname(self):
        '''It should return `True` if the domain has acceptable characters'''

        self.assertTrue(lolram.url.is_allowable_hostname('www.example.com'))
        self.assertFalse(lolram.url.is_allowable_hostname('www.bbéë.com'))
