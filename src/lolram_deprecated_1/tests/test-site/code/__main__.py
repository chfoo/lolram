...
import lolram_deprecated_1.app
import lolram_deprecated_1.components.wui
import lolram_deprecated_1.components.session
import lolram_deprecated_1.components.accounts
import lolram_deprecated_1.components.cms
import lolram_deprecated_1.components.cms.serve
import lolram_deprecated_1.components.database
import lolram_deprecated_1.components.respool
import lolram_deprecated_1.components.accounts.serve
from lolram_deprecated_1.widgets import Document

ABCD = 'abcd'

class SiteApp(lolram_deprecated_1.app.SiteApp):
	def init(self):
	...
	def delete_data(self):
		...
		db = self.context.get_instance(lolram_deprecated_1.components.database.Database)
	...
	def session_test(self):
		session = lolram_deprecated_1.components.session.Session(self.context)
		self.context.response.ok()
	...
	def form_test(self):
		form = lolram_deprecated_1.components.wui.Form()
		doc = self.context.get_instance(lolram_deprecated_1.components.wui.Document)
	...
	def article_text_test(self):
		...
		cms = self.context.get_instance(lolram_deprecated_1.components.cms.CMS)
	...
	def account_basic_test(self):
		...
		acc = lolram_deprecated_1.components.accounts.AccountManager(self.context)
...
#		cms = self.context.get_instance(lolram.components.cms.CMS)
		return lolram_deprecated_1.components.cms.serve.serve(self.context)
	
	def res_pool_text_test(self):
		self.context.response.ok()
		respool = self.context.get_instance(lolram_deprecated_1.components.respool.ResPool)
	...
	def manual_accounts_test(self):
		return lolram_deprecated_1.components.accounts.serve.serve(self.context)
