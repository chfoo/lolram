'''Concatenate scripts and stylesheets into one file controller'''
#
...
#
from lolram.deprecated.web.framework.handlers import SimpleStaticFileHandler
import glob
import logging
import os.path
import pyinotify
...
class ResourceController(torwuf.deprecated.web.controllers.base.BaseController
):
    def init(self):
...
class ScriptsRequestHandler(SimpleStaticFileHandler):
    ...
    def init(self):
        self.root, self.filename = os.path.split(
            self.controller.scripts_file.name)
...
class StylesRequestHandler(SimpleStaticFileHandler):
    ...
    def init(self):
        self.root, self.filename = os.path.split(
            self.controller.styles_file.name)
