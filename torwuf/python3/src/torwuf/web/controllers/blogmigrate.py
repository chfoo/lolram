...
#
from lolram.utils.url import URL
from torwuf.web.controllers.account.authorization.decorators import \
	require_admin
from torwuf.web.models.cms import ArticleCollection, CommentCollection, \
...
class BlogMigrateController(torwuf.web.controllers.base.BaseController):
	def init(self):
		self.add_url_spec('/blog-migrate/url', TitleResolverHandler)
		self.add_url_spec('/blog-migrate/import', ImportHandler)
		

class TitleResolverHandler(torwuf.web.controllers.base.BaseHandler):
	...
	def get(self):
		...
		if url_obj.path.endswith('.html'):
			...
			result = self.app_controller.database[LegacyBlogLookupCollection.COLLECTION].find_one(
				{'_id': title}
			)
...
class ImportHandler(torwuf.web.controllers.base.BaseHandler):
	...
	def post(self):
		...
		for post in d['posts']:
			new_article = {
				...
				ArticleCollection.PUBLICATION_DATE:iso8601.parse_date(post['published']),
				ArticleCollection.TEXT:post['html'],
				ArticleCollection.TITLE:post['title'],
				ArticleCollection.UPDATED_DATE:iso8601.parse_date(post['updated']),
			...
			object_id = self.app_controller.database[ArticleCollection.COLLECTION].insert(new_article)
			article_url_map[post['url']] = object_id
			
			if post['url']:
				self.app_controller.database[LegacyBlogLookupCollection.COLLECTION].insert(
		...
		for comment in d['comments']:
			...
			new_comment = {
				CommentCollection.LEGACY_BLOGGER_ID: comment['blogger_id'].split('/')[-1],
				CommentCollection.PUBLICATION_DATE: iso8601.parse_date(post['published']),
...
#				CommentCollection.LEGACY_ALLOW_RAW_HTML: True,
			...
			self.app_controller.database[CommentCollection.COLLECTION].insert(new_comment)
