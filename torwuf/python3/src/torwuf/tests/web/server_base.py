...
class ServerBaseMixIn(lolram.tests.server_base.ServerBaseMixIn):
	...
	def create_app(self):
		...
		if not sucessful_files:
			raise Exception('No config file read. (tried %s)' % config_filepath)
		
		config_parser['application']['root-path'] = self.root_path
		configuration = lolram.web.framework.app.Configuration(config_parser,
			debug_mode=True)
		self.app_wrapper = torwuf.web.controllers.app.Application(configuration)
		self.app = lolram.web.wsgi.DecodeFromLatin1ToUnicode(self.app_wrapper.wsgi_application)
	...
	host=None, port=None):
		...
		return lolram.tests.server_base.ServerBaseMixIn.request(self, path, method, 
